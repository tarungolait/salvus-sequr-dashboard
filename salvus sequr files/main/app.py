...
def user_post():
    if request.method == 'POST':  
        df3 = request.get_json(force=True)  
        name = df3['Name']
        ...
        if not existing_data:
            ...
            mycursor.execute('insert into infinicue_master_table (barcodeno, qrcode, ble_mac_id, device_id, name, phone, email, passkey, pubkey,privkey,product) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', [barcodeno, obj1[0][0], obj1[0][1], device_id, name, phone, email, final_passkey, public_str, private_str, product])
            mydb.commit()
